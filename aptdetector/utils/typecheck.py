...
_loglevel = logging.ERROR  # logging.LOGLEVEL to use
_exception = True  # exception to throw on type error (eg. TypeError)
...
def _type_error(msg, stack=None):
    ...
    if _loglevel:
        ...
        if instr:
            instr = ': ' + instr
        log_msg = 'File "%s", line %d, in %s: %s%s' % (path, line, in_func,
                                                       msg, instr)
...
def params(**types):
    ...
    def deco(fn):
        ...
        if hasattr(inspect, 'getfullargspec'):
            arg_names, va_args, va_kwargs, _, _, _, _ = inspect.getfullargspec(
                fn)
        else:
